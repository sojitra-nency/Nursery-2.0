...
function CircularSlider({ images, name }: { images: ShowcaseImage[]; name: string }) {
  ...
  // Auto-advance whenever the image set changes.
  useEffect(() => {
        ...
        {images.length > 1 && (
          <>
            <button
              ...
              onClick={() => go(-1)}
              className="absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/45 text-white flex items-center justify-center hover:bg-black/65 transition-colors"
            >
              ‹
            </button>
            <button
              ...
              onClick={() => go(1)}
              className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/45 text-white flex items-center justify-center hover:bg-black/65 transition-colors"
            >
              ›
            </button>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
      <div className="space-y-4">
        <CircularSlider images={active.images} name={active.name || fallbackName} />
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {avail && <Badge tone={availabilityTone(active.availability)}>{avail.label}</Badge>}
        </div>
